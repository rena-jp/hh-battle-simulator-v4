...
import { saveTeamParams } from '../store/team';
...
export async function EditTeamPage(window: Window) {
    ...
    async function saveTeamSimulationParams(window: EditTeamWindow) {
        ...
        const update = (team: any) => {
            const girlArmorCaracs = (team.girls as any[])
                .flatMap(girl => girl.armor.map((armor: any) => toFighterCaracs(armor)))
